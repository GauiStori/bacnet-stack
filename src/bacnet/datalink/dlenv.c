...
static void bip_network_port_activate_changes(uint32_t instance)
{
    ...
    if(update_BBMD_TTL)
    {
        snprintf(buf,sizeof(buf),"%u",BBMD_TTL_Seconds_to_be);
    ...
    {
        snprintf(buf,sizeof(buf),"%u.%u.%u.%u",BBMD_ADDRESS_tobe.host.ip_address.value[0],
        ...
        snprintf(buf,sizeof(buf),"%u",BBMD_ADDRESS_tobe.port);
